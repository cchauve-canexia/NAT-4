...
Two reportable thresholds were considered:
...
The results for minimum VAF=5% are in the directory `VAF_5` and for a minimum 
VAF=1% in the directory `VAF_1`.

The set of all called indels is in the file `ALL_SAMPLES_predicted_VAF_5.vcf`
and `ALL_SAMPLES_predicted_VAF_1.vcf`.

With minimum VAF=5%, all the expected indels are found by the modified pipeline. 
Three additional indels were detected:
